...
def add_rotina(current_user: User, model: RotinaModel):
...
def get_rotina(current_user: User):
    stmt = (
        select(
            Rotina.id, 
            Rotina.nome, 
...
def get_rotina_detalhes(current_user: User, rotina_id: int):
...
def delete_rotina(id: int, current_user: User):
    with Session() as sess:
