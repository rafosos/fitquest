...
from .exercicio_service import get_streaks_geral
...
def get_nao_amigos(user_id: int, filtro: str):
    filtro_string = f"%%{filtro}%%"
...
def change_status_pedido_amizade(
    ...
    with Session() as sess:
        ...
        stmt = select(Amizade).where(and_(Amizade.user1_id == amigo_id, Amizade.user2_id == user_id))
...
def delete_pedido_amizade(user_id: int, amigo_id: int):
    with Session() as sess:
        stmt = select(Amizade).where(Amizade.user1_id == amigo_id, Amizade.user2_id == user_id)
...
def editar_altura(user_id: int, campo: str, valor: str):
    with Session() as sess:
...
def get_perfil_configs(user_id: int):
    with Session() as sess:
