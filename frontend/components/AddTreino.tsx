...
interface Props{
    id: number
    nome: string,
    getExercicios: (id: number) => Promise<ExercicioCampeonatoTreino[]>,
    salvarTreino: (exerciciosId: number[], imagem: string, latitude?: number, longitude?: number) => Promise<AxiosResponse<unknown | any>>
    localizacao?: boolean
}

export default function AddTreino({
    id, nome, getExercicios, salvarTreino, localizacao = true
}: Props){
    ...
    const clearAndClose = () => {
        toast.show("Treino adicionado com sucesso!", {type: 'success'});
        router.back();
    }
